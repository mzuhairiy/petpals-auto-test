...
// CLI entry point — run directly with: npx ts-node src/utils/TestRailPusher.ts <file>
if (require.main === module) {
  const filePath = process.argv[2];

  if (!filePath) {
    console.error('Usage: npx ts-node src/utils/TestRailPusher.ts <path-to-json>');
    console.error('Example: npx ts-node src/utils/TestRailPusher.ts .kilo/snapshots/test-cases/shopping.json');
    process.exit(1);
  }
